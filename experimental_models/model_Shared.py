...
class ConvTemporalGraphical(nn.Module):
    ...
    def __init__(self, time_dim, joints_dim, is_first_layer=False):
...
class ST_GCNN_layer(nn.Module):
    ...
    def __init__(
        ...
        self.gcn = ConvTemporalGraphical(
            time_dim, joints_dim, is_first_layer
...
class Model(nn.Module):
    ...
    def __init__(
        ...
        self.st_gcnns.append(
            ST_GCNN_layer(
                64, 32, [1, 1], 1, input_time_frame, joints_to_consider, st_gcnn_dropout
            )
        )

        self.st_gcnns.append(
            ST_GCNN_layer(
                32, 64, [1, 1], 1, input_time_frame, joints_to_consider, st_gcnn_dropout
            )
        )
        ...
        for i in range(1, n_txcnn_layers):
            self.txcnns.append(
                CNN_layer(
                    output_time_frame, output_time_frame, txc_kernel_size, txc_dropout
                )
            )
